...
- Progress 2026-09-22 release-engineering batch: schema-v1 role manifests verify portable paths, identity, sizes and hashes; staging produces self-contained candidates and refuses invalid/existing destinations. Linux installs now check owned-file inventories, reject malicious archives, roll back failed upgrades and preserve unowned files. Three focused regression runners and the retained real Linux client's package/install/headless-launch/upgrade/uninstall lifecycle pass. CI adds Linux client/editor exports and candidate assembly; hosted execution, native dependency locking, signing and release publication remain open. `[truth:test]` `[truth:runtime]`
...
The [production brief](docs/RELEASE_AND_WORLD_BUILDING_PLAN.md) now has an implemented three-room key/door/objective gate and repaired editor persistence beneath its larger mission, cyclic solver, native-artifact and external-service contracts. Keep finished audiovisual review and release promotion separate from automated gate evidence. `[truth:docs]` `[truth:runtime]`

Every implementation closure updates README, current status/known limits and changelog with the exact exercised gate; the detailed plan retains artifact and external-prerequisite requirements.
...
- Progress 2026-09-11 configuration validation tranche: `ConfigService` now recursively validates schema types, required fields, nested properties/items, enum values, numeric bounds, and minimum lengths instead of unconditionally returning success. Focused proof passes 17/17 with 109 assertions. `[truth:test]` `[truth:source-audit]`
