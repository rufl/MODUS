...
Generator revision **2** changes seed-to-content output. Saved scenes retain seed, configuration, revision and typed gameplay records; replay requires the same generator/runtime/content. Generated/authored content now carries a versioned capability manifest with runtime identity, required features and explicit fallback diagnostics; destination staging and travel admission reject unsupported or mismatched capabilities before instantiation or peer spawn. Key/lock generation now publishes deterministic key, lock, objective, recovery-route and room-edge metadata; disconnected graphs, forged edges and locks not backed by actual adjacency are rejected transactionally, and MissionMgr validates the retained graph contract. Runtime realization of generated records into pickup/door actors, full rule/module/voxel integration and mission presentation/balance review remain open. See the [repair evidence and …
