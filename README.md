...
Generated interactive actors now use explicit collision layers: doors retain world blocking plus interactability, while keys and switches use the interactable layer with no unintended collision mask. Focused actor discoverability proof passes **8/8 tests with 60 assertions**; the broader gameplay regression selection passes **58/58 with 774 assertions**. Native rendered interaction and manual feel remain open.
