...
Windows or `modus` on Linux, plus `README.md` and `LICENSE`. A successful
...
The receiver smoke invokes the packaged client with `--package-smoke`; the
entry point validates the main scene, menu scene, and showcase scene before
exiting. This is a package-integrity gate, not native rendering or gameplay
acceptance.
