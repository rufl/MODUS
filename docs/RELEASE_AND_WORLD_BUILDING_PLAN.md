...
| Progression | Key/door/secret/item records now survive packing; the solver considers all closed locks simultaneously. Records still are not instantiated runtime pickup/door/objective actors or a mission graph. | `generation_context.gd`, `map_generator.gd`, `key_lock_system.gd`, `validation_system.gd` |
...
| Missions and hubs | `MissionMgr` executes authored actor objectives and final prerequisites; the existing game-state manager restores player keys, actors and mission completion. Persistent networked hub travel is not implemented. | `mission_manager.gd`, `game_state_manager.gd`, `level_play_session.gd`, `game/world/world.gd` |
...
| Releases | Five presets include a new Linux standalone editor; Linux editor/game executable exports succeed with installed 4.7.2 templates. Installer, native integration lock, release publication and target-Windows proof remain open. | `export_presets.cfg`, `.github/workflows/ci.yml`, `tools/run_export_smoke.sh` |
...
Persist stable `(module_instance_id, actor_id)` identities, not absolute scene-tree paths. Existing environment save/restore in `game_state_manager.gd` uses NodePaths, and `World._exit_tree` releases the host session. Move session lifetime above replaceable worlds and migrate environment persistence through the existing encrypted transactional save service. Revisiting restores solved doors, defeated one-shot encounters and collected rewards; ordinary travel must not grant them again.
...
The existing `tools/godot/build.sh` pins patched Godot revision `ed1daf0bf001b61586d9930840f2f1394092c079` but builds a Linux editor, not the complete native client/server/export-template set. CI fetches stock export templates and retains its matrix artifacts for 14 days; the job named deploy checks files rather than publishing a release. Packaging therefore needs a real runtime lock and promotion path, not another success banner.
...
- Implementation sequence: generator correctness is delivered; the owner then approved the complete three-room gate. Its module/editor/gameplay implementation and automated evidence are recorded above. Full mission production, persistent hubs and release engineering remain distinct tracked deliveries.
...
- Presentation: the three-room kit implements the proposed coastal/industrial direction with original geometry and materials. In-motion visual/audio review is still required before declaring a finished quality reference or multiplying the full mission kit.
