...
| Steam multiplayer | Steam/ENet transport capability detection now requires both the Steam service and `SteamMultiplayerPeer` class before attempting Steam; peer selection and fallback mode reporting remain explicit. | Two authorized accounts: lobby, invite/join, ownership/auth outcomes, late join, disconnect/reconnect, relay/P2P diagnostics and rejection cases. |
...
The [GodotSteam variant guide](https://godotsteam.com/getting_started/what_are_you_making/) distinguishes standard client and dedicated-server distributions. Verify required singleton methods, signals and `SteamMultiplayerPeer` behavior in each target artifact, not only class presence. `DedicatedServer.init_game_server(...)` now targets `SteamManager.init_game_server(...)`; initialization and auth entrypoints have explicit unavailable/failure results, callback pumping detects client/server API variants, ticket buffers are normalized before RPC validation, and transport selection requires both Steam service and peer-class availability. Missing Steam service permits deliberate offline/ENet selection; a failed Steam session must never report ENet as Steam success.
