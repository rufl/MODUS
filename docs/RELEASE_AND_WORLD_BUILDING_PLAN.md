...
| Progression | Key/door/secret/item records now survive packing; deterministic key-lock generation also publishes versioned objective, lock and recovery-route metadata, rejects impossible placements transactionally and is validated/retained by `MissionMgr`. Generated scenes instantiate canonical enemy, pickup, key, door and extraction actors and retain actor-record/actor-count diagnostics plus a bounded encounter manifest with proportional weapon/ammo/health floors, per-room resource counts and impossible-composition refusal; composed procedural mission graphs, full encounter pacing and human interaction remain separate. | `generation_context.gd`, `map_generator.gd`, `key_lock_system.gd`, `mission_manager.gd`, `validation_system.gd` |
