...
| Module assembly | Canonical `PrefabMetadata` now includes typed sockets, opening/clearance volumes, dimensions, revision and capabilities. `ModuleAssembly` validates transactional placement and bounded generated-plan attachment, including duplicate-room, malformed-transform and empty-plan refusal; the editor library retains that same definition. Full authored catalog diversity and production mission composition remain separate. | `prefab_metadata.gd`, `shared/editor_core/core/module_assembly.gd`, both prefab libraries |
