...
| Releases | Five presets include a new Linux standalone editor; Linux editor/game executable exports succeed with installed 4.7.2 templates. `tools/validate_release_artifacts.py` now emits a versioned SHA-256 manifest for client/server/editor executable-plus-PCK sets and records an optional detached signature; it never claims signing or target-platform runtime proof. Installer, native integration lock, release publication and target-Windows proof remain open. | `export_presets.cfg`, `.github/workflows/ci.yml`, `tools/run_export_smoke.sh`, `tools/validate_release_artifacts.py` |
