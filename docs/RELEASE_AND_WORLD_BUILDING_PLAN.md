...
| Progression | Key/door/secret/item records now survive packing; deterministic key-lock generation also publishes versioned objective, lock and recovery-route metadata, rejects impossible placements transactionally and is validated/retained by `MissionMgr`. Generated scenes instantiate canonical enemy, pickup, key, door and extraction actors and retain actor-record/actor-count diagnostics plus a bounded encounter manifest with proportional weapon/ammo/health floors, per-room resource counts, early/mid/late progression bands and zero spawn-cell collision diagnostics; generated key, door and switch actors expose state-aware prompts through the shared actor contract, the HUD resolves collision children to their actor parents, and generated collision bodies use explicit world/interactable layers; composed procedural mission graphs, full encounter pacing and human interaction remain separate. | `generation_context.gd`, `map_generator.gd`, `gameplay_element_placer.gd`, `shared/editor_core/actors/actor_base.gd`, `shared/editor_core/actors/door_actor.gd`, `shared/editor_core/actors/key_pickup_actor.gd`, `shared/editor_core/actors/switch_actor.gd`, `game/entities/player/components/player_hud_bridge.gd`, `key_lock_system.gd`, `mission_manager.gd`, `validation_system.gd` |
