...
| Progression | Key/door/secret/item records now survive packing; deterministic key-lock generation also publishes versioned objective, lock and recovery-route metadata, rejects impossible placements transactionally and is validated/retained by `MissionMgr`. Generated scenes instantiate canonical enemy, pickup, key, door and extraction actors and retain actor-record/actor-count diagnostics plus a bounded encounter manifest with proportional weapon/ammo/health floors, per-room resource counts, early/mid/late progression bands and zero spawn-cell collision diagnostics; generated key, door and switch actors expose state-aware prompts through the shared actor contract, the HUD resolves collision children to their actor parents, generated collision bodies use explicit world/interactable layers, interact-triggered secret walls are ray-discoverable while other secret triggers retain world/debris blocking, and travel actors expose destination-aware prompts; composed procedural mission graphs, full encounter pacing and human interaction remain separate. | `generation_context.gd`, `map_generator.gd`, `gameplay_element_placer.gd`, `shared/editor_core/actors/actor_base.gd`, `shared/editor_core/actors/door_actor.gd`, `shared/editor_core/actors/key_pickup_actor.gd`, `shared/editor_core/actors/switch_actor.gd`, `shared/editor_core/actors/secret_wall_actor.gd`, `shared/editor_core/actors/travel_actor.gd`, `game/entities/player/components/player_hud_bridge.gd`, `key_lock_system.gd`, `mission_manager.gd`, `validation_system.gd` |
| Module assembly | Canonical `PrefabMetadata` now includes typed sockets, opening/clearance volumes, dimensions, revision and capabilities. `ModuleAssembly` validates transactional placement and bounded generated-plan attachment, including duplicate-room, malformed-transform and empty-plan refusal; `ModuleLayoutSolver` deterministically prefers least-used compatible authored definitions, tries bounded breadth/depth spanning trees for cyclic plans and publishes catalog/strategy diagnostics. Every registered authored catalog definition passes single-room solve/attach coverage. The editor library retains that same definition. Authored-catalog cyclic geometry and complete multi-room content composition remain separate. | `prefab_metadata.gd`, `shared/editor_core/core/module_assembly.gd`, both prefab libraries |
...
| Steam | Steam client and dedicated-server initialization now use explicit availability/result contracts; `DedicatedServer.init_game_server(...)` is backed by `SteamManager.init_game_server(...)`, which dispatches compatible GodotSteam Game Server init/login/heartbeat calls, pumps client/server callbacks through detected API variants, normalizes auth tickets for RPC validation, exposes explicit Steam/ENet transport capabilities, and fails closed when methods or the singleton are unavailable. Real authenticated server login, two-account transport and installed-artifact proof remain external gates. | `game/core/network/dedicated_server.gd:416-449`, `game/core/network/steam_manager.gd:90-730` |
| Workshop | Source lifecycle now binds the GodotSteam singleton, supports both documented camelCase and compatibility snake_case UGC calls, persists local-manifest → PublishedFileId mappings, reuses retained IDs for updates, and separates subscription/downloading/installed state. Real app-owned Workshop configuration, authorized accounts and service transfer evidence remain external prerequisites. | `shared/editor_core/data/workshop_manager.gd`: `_init_steam`, `_steam_upload`, `_on_ugc_item_created`, `_steam_download`, `subscribe` |

The 2026-09-24 handoff slice retains compact `.tar.zst` candidates and adds receiver-compatible Linux `.tar.gz` and Windows `.zip` candidates. `tools/deploy_overzeer_fleet.sh` previews the registered DDJARIN/CHOPPER path and leaves credential resolution to OVERZEER's owner-private `oztok` helper. This still does not establish authenticated deployment, signing, native Windows execution or release promotion.
...
| Steam multiplayer | Steam/ENet transport capability detection now requires both the Steam service and `SteamMultiplayerPeer` class before attempting Steam; module validation shares the common runtime capability contract and destination validation passes detected runtime capabilities into module admission. | Two authorized accounts: lobby, invite/join, ownership/auth outcomes, late join, disconnect/reconnect, relay/P2P diagnostics and rejection cases. |
...
The [GodotSteam variant guide](https://godotsteam.com/getting_started/what_are_you_making/) distinguishes standard client and dedicated-server distributions. Verify required singleton methods, signals and `SteamMultiplayerPeer` behavior in each target artifact, not only class presence. `DedicatedServer.init_game_server(...)` now targets `SteamManager.init_game_server(...)`; initialization and auth entrypoints have explicit unavailable/failure results, callback pumping detects client/server API variants, ticket buffers are normalized before RPC validation, and transport selection requires both Steam service and peer-class availability. Capability admission now shares `FeatureAvailability.COMMON_CAPABILITIES` for module defaults while destination validation uses the detected runtime set. Missing Steam service permits deliberate offline/ENet selection; a failed Steam session must never report ENet as Steam success.
