...
| Steam | Steam client and dedicated-server initialization now use explicit availability/result contracts; `DedicatedServer.init_game_server(...)` is backed by `SteamManager.init_game_server(...)`, which dispatches compatible GodotSteam Game Server init/login/heartbeat calls, pumps client/server callbacks through detected API variants, normalizes auth tickets for RPC validation, and fails closed when methods or the singleton are unavailable. Real authenticated server login, two-account transport and installed-artifact proof remain external gates. | `game/core/network/dedicated_server.gd:416-449`, `game/core/network/steam_manager.gd:90-520` |
...
The [GodotSteam variant guide](https://godotsteam.com/getting_started/what_are_you_making/) distinguishes standard client and dedicated-server distributions. Verify required singleton methods, signals and `SteamMultiplayerPeer` behavior in each target artifact, not only class presence. `DedicatedServer.init_game_server(...)` now targets `SteamManager.init_game_server(...)`; initialization and auth entrypoints have explicit unavailable/failure results, callback pumping detects client/server API variants, and ticket buffers are normalized before RPC validation. Missing Steam service permits deliberate offline/ENet selection; a failed Steam session must never report ENet as Steam success.
