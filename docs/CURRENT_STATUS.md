...
The focused capability/prefab/mission selection passes **43/43 tests with 299 assertions**; the deterministic generator selection passes **6/6 with 191 assertions**. Planner/key-lock graph regressions pass **14/14 tests with 48 assertions**, generator actor/composition realization and impossible-resource refusal passes **22/22 tests with 544 assertions**, standalone package overwrite protection passes **6/6 with 17 assertions**, authored mission validation passes **6/6 with 48 assertions**, and spatial solver/catalog/attachment selection passes **10/10 with 100 assertions**. The generated 32×32 LevelRoot smoke now requires a valid spatial plan, preserved attached connection metadata, catalog usage diagnostics, exact actor realization diagnostics and a valid encounter manifest. This proves source and headless contracts only. Full encounter pacing, runtime interaction/presentation, production mission composition, authored cyclic geometry and unsupported cyclic layouts remain open.
Encounter placement now refuses shared actor/resource grid cells and retains early/mid/late progression bands plus collision diagnostics in packed gameplay metadata. The focused placer proof passes **13/13 tests with 75 assertions**; the combined generator selection passes **85/85 with 1,025 assertions**. Full encounter tuning, runtime interaction feel, composed mission production and graphical/audiovisual acceptance remain open.
Generated key, door and switch actors now expose state-aware interaction prompts through the shared actor contract. The player HUD resolves collision children back to their actor parent, so generated actors no longer need a parallel `Interactable` component solely for prompt discovery. Authored mission interaction proof passes **7/7 tests with 54 assertions**; the combined gameplay/editor selection passes **47/47 with 267 assertions**. Human-operated feel and rendered acceptance remain open.
Generated door collision leaves now carry both world-blocking and interactable layers; key and switch bodies are interactable-only with zero collision masks. This makes generated actor prompts physically discoverable without eroding player/world collision. Focused actor proof passes **8/8 tests with 60 assertions**; the broader gameplay regression selection passes **58/58 with 774 assertions**. Native rendered interaction and manual feel remain open.

Interact-triggered secret walls now use world plus interactable collision layers while channel, proximity and shootable walls retain world/debris blocking. Travel actors expose destination-aware prompts through the same contract. Focused actor discoverability proof covers **9/9 tests with 66 assertions**; native rendered interaction and manual feel remain open.
