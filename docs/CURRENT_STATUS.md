...
The owner selected a **bundled full-feature runtime**; no Steamworks app exists yet. Remaining generator rule/voxel/module integration, automatic realization of generated gameplay records, mission presentation/balance acceptance, hubs, release packaging and external-service proof stay open.
...
Black Start's packaged source-game smoke passes **68 checks**: real navigation-guided player traversal, 14 actual enemy deaths, gated power/cooling/transmission, mid-encounter and mid-lift encrypted save/load, powered hub return and completed-state restoration without duplicated enemies or loot. Optional discoveries do not block required completion; separate cache playback verifies real hitscan, physical access, healing and non-renewing rewards after load. The focused mission/actor/editor/package/navigation selection passes **55/55 tests, 283 assertions**. Linux client/editor exports and startup smokes pass, including native `.mdsl` entry; full mission completion remains source-game proof. Four isolated llvmpipe room captures verify rendering and repaired HUD context ownership/restoration after the initial pressure deferral. Local-only evidence is retained under `logs/breakwater_mission/`. Combat defeats are driven through `CombatSvc`; these observations do not establish human-operated completion, audiovisual approval or pacing.
